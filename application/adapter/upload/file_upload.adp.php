...
class file_upload
{
	...
	function upload($field,	$fileName ,$filePath=false, $fType='jpg,jpeg,gif,png',	$maxSize=MAX_UPLOAD_FILE_SIZE){
        ...
        if(!is_dir($filePath)) {
            // 检查目录是否编码后的
        	if(!mkdir($filePath)){
               $this->fileInfo['errmsg']  =  L('adapter__load__fileUpload__uploadDirNotExist', $filePath);
               $this->fileInfo['errcode'] = 40050;
               return false;
            }
        }else{
        	if(!is_writable($filePath)){
        		$this->fileInfo['errmsg']  =  L('adapter__load__fileUpload__uploadDirNotWritable', $filePath);
        		$this->fileInfo['errcode'] = 40050;
        		return false;
        	}
        }
	...
	function _check($file) {
        ...
        if(!$this->_checkType($file['name'])) {
            $this->fileInfo['errmsg'] = L('adapter__load__fileUpload__uploadFileMine');
