...
class localpm_service{
	 ...
	 * )
	 */
	function listGroupByUid($sina_uid, $page = 1, $count = 50){
		$result = array('user_list'=>array(), 'total_number'=>0);
		
		$result['total_number'] = DS('localpm_reader.pm_index_user_countOneUser', null, $sina_uid);
		if($result['total_number'] < 1){
			return RST($result);
		}
		
		$iidList = DS('localpm_reader.pm_index_user_getListGroupByUid', null, $sina_uid, $page, $count, true);
		...
		foreach($iidList as $data){ 
			...
			$r['iid'] = $data['iid'];
			$r['user_id'] = strtr($data['actors'], array($sina_uid. '#' => '', '#'. $sina_uid=>''));
	 ...
	 * @param bool $ignore_from_uid_check 是否忽略发消息人的合法性检查(默认true)？如果第一个参数不能保证为当前登录用户的sina_uid，请设置为false
	...
	function writeNew($from_uid, $to_uid, $text, $ignore_from_uid_check = true, $ignore_to_uid_check = false){
		...
		if(0 != $disallow_not_friend_local_pm){
			$res = DR('xweibo/xwb.existsFriendship', null, $to_uid, $from_uid);
	 ...
	 * @param bigint $from_uid 哪个人（sina_uid）发消息？
	 ...
	 * @return RST_array
	 */
	function resetUnread($sina_uid, $force = false){
		if(!is_numeric($sina_uid)){
			return RST(false);
		}
		DR('common/userConfig.set', null, 'new_local_pm', 0, $sina_uid);
		
		if(true == $force){
			DR('localpm_writer.pm_index_user_reset_unread_all', null, $sina_uid);
		} 
		return RST(true);
	}
