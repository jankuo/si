...
class events
{
    ...
    function getMineAttendEvents($uid, $page = 1, $limit = 50) {
        ...
        $sql = 'SELECT e.id, e.title, e.addr, e.desc, e.cost, e.sina_uid, e.nickname, e.realname, e.phone, e.start_time, e.end_time, e.pic, e.wb_id, e.join_num, e.state, e.other FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' AS j LEFT JOIN ' . $this->table . ' AS e ON j.event_id = e.id WHERE j.sina_uid = "' . $this->db->escape($uid) . '" ORDER BY e.add_time DESC' . $limit_str;

        $data = $this->db->query($sql);
        for ($i=0, $count=count($data); $i<$count; $i++) {
            $data[$i]['state_num'] = $this->getState($data[$i]);
        }
        $this->count_sql = 'SELECT COUNT(*) FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' AS j LEFT JOIN ' . $this->table . ' AS e ON j.event_id = e.id WHERE j.sina_uid = "' . $this->db->escape($uid) . '"';
        return RST($data);
    }
    ...
    function getMineAttendEventsCount($uid) {
        $sql = 'SELECT COUNT(*) FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' AS j LEFT JOIN ' . $this->table . ' AS e ON j.event_id = e.id WHERE j.sina_uid = "' . $this->db->escape($uid) . '"';
        return RST($this->db->getOne($sql));
    }
    ...
    function deleteEvent($eid) {
        $sql = 'DELETE FROM ' . $this->table . ' WHERE id = \'' . $this->db->escape($eid). '\'';
        $ret = $this->db->execute($sql);

        if ($ret) {
            /// 删除参加该活动的数据
            $sql = 'DELETE FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' WHERE event_id = "' . $this->db->escape($eid) . '"';
            $this->db->execute($sql);
    ...
    function deleteEventComment($eid, $wb_id) {
        $sql = 'DELETE FROM ' . $this->db->getTable(T_EVENT_COMMENT) . ' WHERE event_id = "' . $this->db->escape($eid) . '" AND wb_id = "' . $this->db->escape($wb_id) .'"';
        $ret = $this->db->execute($sql);
    ...
    function isJoinEvent($event_id, $sina_uid)
    {
        if (is_array($event_id)) {
            $where = ' event_id in (' . $this->db->SQLIN($event_id) . ')';
        } else {
            $where = 'event_id = "' . $this->db->escape($event_id). '"';
        }
        
        $sql = 'SELECT event_id, sina_uid FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' WHERE ' . $where . '  AND sina_uid = "' . $this->db->escape($sina_uid) . '"';
        $data = $this->db->query($sql);
        return RST($data);
    }
    ...
    function getMembers($event_id, $rows, $offset = 0) {
        $sql = 'SELECT * FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' WHERE `event_id`=\'' . $this->db->escape($event_id) . '\' LIMIT ' . $offset . ',' . $rows;
        $members = $this->db->query($sql);
        $this->count_sql = 'SELECT count(*) FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' WHERE `event_id`= "' . $this->db->escape($event_id) . '"';
        return RST($members);
    }
    ...
    function getMembersCount($event_id) {
        $sql = 'SELECT count(*) FROM ' . $this->db->getTable(T_EVENT_JOIN) . ' WHERE `event_id`= "' . $this->db->escape($event_id) . '"';
        return RST($this->db->getOne($sql));
    }
    ...
    function getEventComments($eid, $page = 1, $limit = 20) {
        $offset = ($page - 1) * $limit;
        $sql = 'SELECT c.wb_id, c.weibo FROM ' . $this->db->getTable(T_EVENT_COMMENT) . ' AS c LEFT JOIN ' . $this->table . ' as e ON c.event_id = e.id WHERE c.event_id = \'' . $this->db->escape($eid) . '\' LIMIT ' . (int)$offset . ', ' . $limit;
        $this->count_sql =  'SELECT count(*) FROM ' . $this->db->getTable(T_EVENT_COMMENT) . ' AS c LEFT JOIN ' . $this->table . ' as e ON c.event_id = e.id WHERE c.event_id = "' . $this->db->escape($eid) . '"';
        return RST($this->db->query($sql));
    }
...
//		DD('events.getMineAttendEvents');
    ...
    function getEventComments2($eid) {
        $sql = 'SELECT c.wb_id, c.weibo FROM ' . $this->db->getTable(T_EVENT_COMMENT) . ' AS c LEFT JOIN ' . $this->table . ' as e ON c.event_id = e.id WHERE c.event_id = \'' . $this->db->escape($eid) . '\' AND (c.weibo = "" || ISNULL(c.weibo))';
        return RST($this->db->query($sql));
    }
}
