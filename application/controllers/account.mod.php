...
class account_mod {
	...
	function sinaLogin(){
		...
		//登录验证码
		if(IS_USE_CAPTCHA && (!defined('DISABLE_SINALOGIN_CAPTCHA') || DISABLE_SINALOGIN_CAPTCHA != true)){
			$this->_checkloginCatcha();
		}
	...
	function _checkloginCatcha(){
		$checkResult = false;
		$err_msg = '';
		
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			if(!isset($_SESSION['login_formhash']) || empty($_SESSION['login_formhash'])){
				$err_msg = '内部错误或者提交方式存在问题，请重新输入';
			}elseif($_SESSION['login_formhash'] != V('p:formhash')){
				$err_msg = '表单验证失败，请重新输入';
			}else{
				$autocode = APP :: N('SimpleCaptcha');
				$verify_code = strval(V('p:verify_code'));
				if (!$autocode->checkAuthcode($verify_code)) {
					$err_msg = '验证码输入错误，请重新输入';
				}else{
					$_SESSION['login_formhash'] = '';
					$checkResult = true;
				}
			}
		}
		...
		if(true != $checkResult){
			$rand = function_exists('mt_rand') ? mt_rand(1, 10000000) : rand(1, 10000000);
			$rand_offset = function_exists('mt_rand') ? mt_rand(0, 14) : rand(0, 14);
			$newFormhash = substr(md5(WB_SKEY. APP_FLAG_VER. time(). $rand. WB_SKEY), $rand_offset, 16);
			$_SESSION['login_formhash'] = $newFormhash;
			TPL::module('checkloginCatcha_input', array('err_msg'=>$err_msg, 'formhash'=>$newFormhash));
			exit();
		}
